...
TEST(PolyTest, GeneralSetup)
{
	...
	double error_tolerance = cv::norm(expected_result) / 100.0 / 2.0;
	EXPECT_NEAR(result.x, expected_result.x, error_tolerance);
	EXPECT_NEAR(result.y, expected_result.y, error_tolerance);
	EXPECT_NEAR(result.z, expected_result.z, error_tolerance);
}
